...
export function ReconciliationPage() {
  ...
  const [trip, setTrip] = useState<ReconciliationTripId>(() => workspace?.periods[0]?.tripId ?? "workspace");
  ...
  useEffect(() => {
    ...
    setImportSource(source?.id ?? `statement-${trip}`);
  ...
  const cash = cashSummary(workspace, Math.round((Number(endingCash) || 0) * 100), trip);
  ...
  function editTransaction(id: string, patch: Partial<ReconciliationTransaction>) {
    ...
    if (updated && expense) {
      ...
      setNotice("Updated the ledger transaction and linked group expense.");
      return;
    }
  ...
  function addStatementToTrip(transaction: ReconciliationTransaction) {
    ...
    const linkedTransaction = workspace.transactions.find((item) => item.tripId === transaction.tripId && item.side === "left");
    const linkedExpense = linkedTransaction ? state.expenses.find((item) => item.id === linkedExpenseId(linkedTransaction)) : undefined;
    const group = state.groups.find((item) => item.id === linkedExpense?.groupId);
    if (!group) {
  ...
  function createReconciliation(name: string, dates: string) {
    ...
    const sourceIds = { wanderlog: `ledger-${id}`, statement: `statement-${id}` };
    const next: ReconciliationWorkspace = {
      ...workspace,
      sources: [
        ...workspace.sources,
        { id: sourceIds.wanderlog, tripId: id, type: "wanderlog", institution: "Shared ledger", account: "Trip expenses", currency: "CAD", importedAt: new Date().toISOString(), fingerprint: id },
  ...
  function exportPackage() {
    const packageData = {
      ...
      totals,
      cashSummary: cash.total > 0 ? cash : undefined,
      sources,
      transactions,
  ...
  function flagSuggestion(group: ReconciliationMatchGroup, note: string) {
    if (isLocked) return;
    const ids = [...group.leftIds, ...group.rightIds];
    const amountCents = Math.abs(group.leftTotalCents - group.rightTotalCents);
    const next: ReconciliationWorkspace = {
      ...workspace,
      transactions: workspace.transactions.map((item) => ids.includes(item.id) ? { ...item, status: "exception" as const } : item),
    ...
    updateWorkspace(withAudit(next, "support", note, ids));
    setNotice(note);
  }
      ...
      <main className="pane pane-wide reconciliation-page simple-reconciliation">
        ...
        <section className="simple-match-list" aria-labelledby="likely-matches-title">
          ...
          {strongSuggestions.map((group) => {
            ...
            return (
              <article key={group.id} className="simple-match-row">
  ...
  return (
    <main className="pane pane-wide reconciliation-page">
      ...
      <section className="recon-overview">
        ...
        <Metric label="Trip ledger" value={money(totals.left)} detail={`${totals.leftCount} items`} />
      ...
      {cash.total > 0 && (
        <section className="cash-control cash-summary">
          ...
          <div className="cash-equation">
            <Metric label="Opening cash" value={money(cash.opening)} detail="First cash entry" />
            <span>+</span>
            <Metric label="Additional cash" value={money(cash.withdrawals)} detail="Imported cash activity" />
            <span>−</span>
            ...
            <span>=</span>
            <Metric label="Cash used" value={money(cash.used)} detail="Match this spending to ledger activities" />
          </div>
        </section>
      )}
  ...
  return (
    <section className="recon-ledger">
      ...
      <div className="recon-lines">
        ...
        {rows.length === 0 && <p className="recon-empty">No {side === "left" ? "ledger" : "statement"} transactions in this view.</p>}
      </div>
    </section>
  );
}
...
function SuggestionList({
  ...
  return (
    <section className="recon-suggestions">
      <header><h2>Suggestions ({suggestions.length})</h2><span>Exact CAD any date · rounded ledger entries ±7 days · exact groups up to 3</span></header>
        ...
        return (
          <article key={group.id}>
            ...
            <div className="suggestion-side">
              <small>Trip ledger · {group.leftIds.length}</small>
      ...
      {visible.map((group) => (
        <details key={group.id} open>
          ...
          <div className="recon-group-body">
            <GroupColumn label="Trip ledger" ids={group.leftIds} byId={byId} />
  ...
  return (
    <section className="recon-import">
      ...
      <div className="recon-import-controls">
        <label>Trip<input value={trip.replace(/[-_]/g, " ")} disabled /></label>
        <label>Mapping
