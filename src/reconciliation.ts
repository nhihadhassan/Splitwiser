...
import { splitByWeights, splitEqually } from "./utils/money.js";
...
function baseSources(tripIds: ReconciliationTripId[]): ReconciliationSource[] {
  return tripIds.flatMap((tripId) => [
    source(`ledger-${tripId}`, tripId, "wanderlog", "Shared ledger", "Group expenses"),
    source(`statement-${tripId}`, tripId, "bank", "Card statement", "Imported transactions"),
    source(`export-${tripId}`, tripId, "import", "Account export", "Imported transactions"),
    source(`cash-${tripId}`, tripId, "cash", "Cash", "Cash transactions"),
  ]);
}
...
function reconciliationTripForGroup(
  workspace: ReconciliationWorkspace,
  groupId: string | null,
  expenses: Expense[] = [],
): ReconciliationTripId | null {
  if (!groupId) return null;
  const direct = workspace.transactions.find((item) => item.side === "left" && item.raw.groupId === groupId);
  if (direct) return direct.tripId;
  const siblingIds = new Set(expenses.filter((expense) => expense.groupId === groupId).map((expense) => expense.id));
  const sibling = workspace.transactions.find((item) => {
    const expenseId = linkedExpenseId(item);
    return expenseId ? siblingIds.has(expenseId) : false;
  });
  return sibling?.tripId ?? tripIdForGroup(groupId);
}
...
export function syncExpenseToReconciliation(
  ...
  const tripId = reconciliationTripForGroup(state.workspace, expense.groupId, expenses);
...
function buildTransactions(state: ReconciliationState, expenses: Expense[]): ReconciliationTransaction[] {
  ...
  const expenseTrips = expenses
    .filter((expense) => expense.groupId)
    .map((expense) => tripIdForGroup(expense.groupId!));
  const tripIds = [...new Set<ReconciliationTripId>([
    ...Object.keys(state.cardTransactions),
    ...Object.keys(state.exportTransactions),
    ...expenseTrips,
  ])];
  for (const [tripIndex, tripId] of tripIds.entries()) {
    expenses
      .filter((expense) => expense.groupId && tripIdForGroup(expense.groupId) === tripId)
      .forEach((expense) => result.push(leftTransaction(expense, tripId, legacyDecision(state, tripId, expense.id, "left"))));
    (state.cardTransactions[tripId] ?? []).forEach((item) => {
      result.push(rightTransaction(item, tripId, "statement", legacyDecision(state, tripId, item.id, "right")));
    });
    (state.exportTransactions[tripId] ?? []).forEach((item) => {
    ...
    if (tripIndex === 1) {
      state.secondaryCashTransactions.forEach((item) => {
        result.push(rightTransaction(item, tripId, "cash", legacyDecision(state, tripId, item.id, "right")));
      });
    }
  }
...
function migrateGroups(
  ...
  Object.entries(state.matches).forEach(([legacyLeft, legacyRightIds]) => {
    ...
    const matchingLeft = transactions.find((item) => item.side === "left" && legacyLeft.endsWith(item.reference));
    if (!matchingLeft) return;
    const tripId = matchingLeft.tripId;
    const expenseId = matchingLeft.reference;
...
export function createReconciliationWorkspace(
  ...
  return {
    ...
    rules: [
      {
        id: "default-exact",
        name: "Exact or rounded ledger amounts within 7 days",
        priority: 10,
        sourceIds: [],
        dateToleranceDays: 7,
    ...
    importMappings: [
      ...
      { id: "trip-ledger", name: "Trip ledger", sourceType: "wanderlog", columns: { date: "date", description: "description", amount: "amount" } },
...
export function importedTransaction(
  ...
  return transaction({
    ...
    sourceId,
    tripId,
    side: sourceId.startsWith("wanderlog") || sourceId.startsWith("ledger") ? "left" : "right",
    accountType: sourceId.startsWith("wanderlog") || sourceId.startsWith("ledger") ? "wanderlog" : "card",
    date: row.date,
    postedDate: row.date,
...
export function cashSummary(workspace: ReconciliationWorkspace, endingCashCents: number, tripId?: ReconciliationTripId) {
  const cash = workspace.transactions.filter((item) => (!tripId || item.tripId === tripId) && item.accountType === "cash" && item.status !== "excluded");
  const opening = cash.slice(0, 1).reduce((sum, item) => sum + item.postedCadCents, 0);
  const withdrawals = cash.slice(1).reduce((sum, item) => sum + item.postedCadCents, 0);
